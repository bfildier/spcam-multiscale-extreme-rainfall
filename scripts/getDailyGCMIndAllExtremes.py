...
def parseArguments():
	...
	return args.pr_id, args.directory, args.compset, args.experiment, \
...
if __name__ == "__main__":
	...
	pr_id, dataroot, compset, experiment, subsetName, datetimes =  parseArguments()
	if compset == 'FAMIPC5':
		model = 'CESM111-CAM5'
	else:
	...
	rootgrp.description = ("GCM points with %s values at each "+\
	"percentile for the %s subdomain and %s experiment.")%\
	(pr_id,subsetName,experiment)
	...
	newvar = rootgrp.createVariable(newvarid,"i1",("time","lat","lon"))
	...
	newvar.long_name = "Index of %s percentiles"%(pr_id)
	newvar.units = ""
	...
	newvalues = -np.ones(pr_vals.shape)
	# Get precipitation boundaries
	pr_q = df_perc[pr_id]

	for i in range(len(pr_q)):
		...
		# Keep index on right locations
		newvalues[loc_perc_subset] = i
	# Store values
	newvar[:] = newvalues[:]
