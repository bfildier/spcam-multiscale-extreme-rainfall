...
#                                                                              #
#    From GCM daily (ie. mesoscale) values of PS and convective-scale values   #
#    of T (CRM-T-IXX), compute daily vertical profiles of QVSATENV in each     #
#    GCM grid cell (name it QVSATENV_CRM_T_IXX).                               #
#    If the fraction entered if a negative integer, use the mesoscale variable #
#    T and compute the daily GCM-scale variable QVSATENV_T.                    #
#                                                                              #
################################################################################
...
from thermodynamics import saturationSpecificHumidity,g
...
if __name__ == "__main__":
	...
	if fraction < 0:
		temp_id = "T"
		qvstar_id = "QVSATENV_T"
	else:
		indvarid = "I%d"%fraction
		temp_id = 'CRM_T_%s'%indvarid
		qvstar_id = "QVSATENV_CRM_T_%s"%indvarid

	##-- Output metadata --##

	outputdir = defineOutputDirectory(dataroot,compset,experiment)
	outputfile = "%s_day_CESM111-SPCAM20_%s_r1i1p1_%s-%s.nc"%\
		(qvstar_id.replace('_','-'),experiment,datetime1,datetime2)
	...
	qvstar_dims = fh_source.variables[ref_id].dimensions
	...
	qvstar = rootgrp.createVariable(qvstar_id,"f4",qvstar_dims)
	...
	qvstar.long_name = str("Mean saturation specific humidity")
	qvstar.units = str("kg/kg")
	...
	temp_values = getVar(temp_id,inputdir_processed_day,dates=(datetime1,datetime2))
	pressurf_values = getVar(pressurf_id,inputdir_processed_day,dates=(datetime1,datetime2))
	...
	qvstar_values = saturationSpecificHumidity(temp_values,pres_values)
