...
#                                                                              #
#    From GCM hourly values of T and Ps, compute vertical profile of satura-   #
#    tion specific humidity in each GCM grid cell, name it QVSATENV (because   #
#    use the environmental temperature profile to compute it.                  #
#                                                                              #
################################################################################
...
def parseArguments():
	...
	return args.experiment, args.compset, args.dataroot, datetime1, datetime2, ncfiles, args.dataroot
...
def defineOutputDirectory(dataroot,compset,experiment):
	...
	case = "bf_%s_%s"%(compset,experiment)
	if hostname == "jollyjumper":
		outputdir = os.path.join(dataroot,'preprocessed',case,'1hr')
	elif "edison" in hostname or "cori" in hostname:
...
if __name__ == "__main__":
	...
	experiment, compset, case, datetime1, datetime2, ncfiles, dataroot = parseArguments()
	...
	outputfile = "%s_1hr_CESM111-SPCAM20_%s_r1i1p1_%s-%s.nc"%\
		(newvar_id,experiment,datetime1,datetime2)
	...
	newvar_dims = fh_source.variables[temp_id].dimensions
	newvar_shape = fh_source.variables[temp_id].shape
	...
	rootgrp.description = "Hourly %s values from CESM111-SPCAM20 in the %s scenario."%\
	...
	newvar = rootgrp.createVariable(newvar_id,"f4",newvar_dims)
	...
	newvar.long_name = str("Saturation specific humidity from GCM-scale environmental profiles.")
	...
	temp_values = np.zeros((n,newvar_shape[1],nlat/3,newvar_shape[3]))
	pressurf_values = np.zeros((n,nlat/3,newvar_shape[3]))
	...
	##-- Compute hourly qvstar profile at each GCM point --##
