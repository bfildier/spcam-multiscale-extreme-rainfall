...
#                                                                            #
#    From hourly values of CRM_T and CRM_W (known at the CRM-scale), hourly  #
#    values of CRM_Q and PS (known at the GCM-scale) and indices IXX,        #
...
if __name__ == "__main__":
	...
	omega_id = "CRM_OMEGA_WT_%s"%indvarid
	...
	temp_values = np.zeros((n,)+w_shape[1:-2]+(nlat/3,w_shape[-1]))
	spechum_values = np.zeros((n,ref_shape[1],nlat/3,ref_shape[3]))
	pressurf_values = np.zeros((n,nlat/3,ref_shape[3]))
	...
	# Add crm_y and crm_x dimensions to spechum
	spechum_values = np.moveaxis(np.repeat(np.repeat(spechum_values[...,np.newaxis],\
												ny,4)[...,np.newaxis],\
												nx,5),\
								[-2,-1],[2,3])
	...
	pres_values = np.moveaxis(np.repeat(np.repeat(pres_values[...,np.newaxis],\
												...
												nx,5),\
								[-2,-1],[2,3])
	# No need to extend vertical velocity values
	# But need to reverse order in the vertical
	w_values = w_values[:,::-1,...]
