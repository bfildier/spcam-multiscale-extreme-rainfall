...
#    locations of CRM_PREC_IXX quantile $Q_ID outside the fraction area        #
...
##-- Define variable, new file name --##
	...
	newvarid = "CRM_MFdry_%s_%s_%s"%(area_id,pr_id.replace('-','_'),Q_id)
...
##-- Create and initialize new file --##
	...
	rootgrp.description = "Mass flux values at CRM points contributing to %s's %2.4fth percentile in the dry area"%\
		(pr_id,Qs[J])

	# Define variables
	is_new_cell = rootgrp.createVariable("isNewCell","u1",("time","rank"))
	crm_mf = rootgrp.createVariable(newvarid,"f8",("time","rank"))
	crm_mfup = rootgrp.createVariable(newvarid.replace('MFdry','MFdryUP'),"f8",("time","rank"))
	crm_mfdn = rootgrp.createVariable(newvarid.replace('MFdry','MFdryDN'),"f8",("time","rank"))
	crm_mf_pw = rootgrp.createVariable(newvarid.replace('MFdry','MFdry_PW'),"f8",("time","rank"))
	crm_mfup_pw = rootgrp.createVariable(newvarid.replace('MFdry','MFdryUP_PW'),"f8",("time","rank"))
	crm_mfdn_pw = rootgrp.createVariable(newvarid.replace('MFdry','MFdryDN_PW'),"f8",("time","rank"))
	crm_pw = rootgrp.createVariable(newvarid.replace('MFdry','PW'),"f8",("time","rank"))
